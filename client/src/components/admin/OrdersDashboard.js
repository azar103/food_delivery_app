...
import { fetchItems, RemoveFromCart } from "../../reducers/cartActions";
function OrdersDashboard(props) {
  ...
  const filteredOrdersByFirstName = props.items.filter(
    (item) =>
      item.food.name.toLowerCase().indexOf(inputValue.toLowerCase()) !== -1
  );
  const filteredFoodsByNameAndCity = props.items.filter(
    (order) =>
  ...
  const cartfilteredByNameSorted = filteredOrdersByFirstName.sort(
    (a, b) => b.date - a.date
  );
  const cartfilteredByNameAndCitySorted = filteredFoodsByNameAndCity.sort(
    (a, b) => b.date - a.date
  );
          ...
          <TableModal>
            ...
            {selectedValue !== "select a city"
              ? cartfilteredByNameAndCitySorted.map((cart, index) => (
                  <OrderDashboard key={index} cart={cart} onDelete={onDelete} />
                ))
